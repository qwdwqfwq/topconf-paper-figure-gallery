...
## 5d. v0.6：跨代去重 / Cross-generation dedup

v0.6 复核发现 v1 人工底（60 张，旧短编号如 `icml2023-12`）与后续管线选图（4 位编号如
`icml2023-0627`）对**同一论文**存在重复收录，共 12 组（ICLR 3、ICML 4、NeurIPS 5）。
...
用户勾选的 1–3 张参考图，会按生成路径以不同方式真正送入模型，而不是只传一个文字名：

- **SVG 路径**：把每张参考图读取为 base64，作为多模态消息的 `image_url`（OpenAI 兼容协议）
  或 `image` content block（Anthropic 协议）与文字 prompt 一起发送；模型真正"看到"参考图像素。
  模型不支持视觉 / 多模态请求失败时，自动降级为只发送参考图的文字标签（标题 + 会议 + pattern）。
- **位图路径**：火山方舟 Seedream 用 `image: [uris]` 传多张参考图（多图参考）；
  硅基流动用单图参考；CogView 等不支持参考的模型自动降级为无参考纯 prompt。

**2. 图型（teaser / pipeline / framework …）如何区分**

类型区分**不是模型自动总结的**，而来自三处人工设计的共同作用：

1. 画廊在收录时为每张图预打了 `pattern` 标签（teaser / conceptual / framework / pipeline /
   architecture / taxonomy），并在检索结果与参考条目中显示；
2. 页面内置 `TARGETS` 字典，为每一种目标图型手写一段**版式指令**（面板如何切、阅读方向、
   箭头与标签用法），生成时按用户选择注入；
3. 多模态输入时，模型对参考图做 **few-shot 模仿**——直接学习所选图的版式骨架。

因此模型无需"自己判断 teaser 与 framework 的区别"：版式指令已显式给出，参考图提供了范例。

**3. 选同类型参考图为什么更准**

想画 framework 就优先勾选带 `framework` 标签的参考图，模型通过多模态输入能直接观察到该类
版式的共同特征（中心向外、模块围绕核心、分组配色），生成结果更贴合目标图型；这比混入不同
类型参考图（版式语言互相冲突）更稳定。建议勾选 1–2 张同类型、高评分参考图。
