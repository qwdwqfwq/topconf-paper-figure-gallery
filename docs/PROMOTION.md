...
- Thousands of curated Figure 1 / teaser figures from **ICLR, ICML, NeurIPS, CVPR, ACL and AAAI (2023–2025)**
...
**It's not a raw figure dump.** Every figure is cropped from the officially published PDF and passes a design-quality pipeline: 25+ heuristic rules explicitly reject default matplotlib charts, plain tables, GUI screenshots, unlabeled photo dumps and result grids; perceptual hashing removes duplicates; then multi-seed sampled visual QA (contact sheets) cleans the remainder. The pipeline is fully open and reproducible (`scripts/`, `docs/METHODOLOGY.md`).

A few honest notes:
- Counts differ across venues on purpose — CV/vision-heavy venues simply contain more *designed* overview figures than venues with more default charts. Quality > quantity; I didn't pad any venue to a round number.
...
A searchable/filterable gallery of curated Figure 1 / teaser figures from ICLR, ICML, NeurIPS, CVPR, ACL and AAAI (2023–2025), meant as an inspiration reference when you design your own paper's overview figure.

https://qwdwqfwq.github.io/topconf-paper-figure-gallery/

It's a pure static site (no backend, works offline). Each figure is cropped from the published PDF and filtered by ~25 heuristic design-quality rules (default charts, tables, screenshots and photo dumps are rejected), perceptual-hash de-duplicated, then visually QA'd in sampled contact sheets. The full pipeline is open source:
...
4. 技术与态度：不是简单抓第一张图——PDF 渲染裁 Figure 1、25+ 条设计质量规则剔除默认图表/表格/截图/照片墙、dHash 去重、配额选图、多随机种子拼图人工 QA；各会议数量不硬凑，如实说明设计图占比差异；管线完全开源可复现。
...
您好，我做了一个开源项目 Top-Conf Figure Gallery：把 ICLR、ICML、NeurIPS、CVPR、ACL、AAAI 六大顶会 2023–2025 年中经过设计质量筛选的 Figure 1 / Teaser 集中为可搜索、可按视觉模式筛选的在线画廊，并开源了从会议索引、PDF 裁图到 25+ 条设计规则打分、人工抽检 QA 的完整可复现管线。
