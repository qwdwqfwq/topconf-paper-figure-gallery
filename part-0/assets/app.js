...
(function () {
  ...
  const PATTERNS = {
    teaser:       "Teaser 主视觉",
    conceptual:   "Conceptual 概念示意",
    framework:    "Framework 框架总览",
    pipeline:     "Pipeline 流程图",
    architecture: "Architecture 架构图",
    taxonomy:     "Benchmark / 任务全景",
    results:      "Results 结果对比",
  ...
  function cardHtml(f) {
    ...
    <article class="card" data-id="${f.id}" tabindex="0" role="button" aria-label="查看 ${escapeHtml(f.title)}">
      ...
      <div class="card-body">
        ...
        <span class="card-link">查看大图 / 论文 ↗</span>
      </div>
    </article>`;
  }
  ...
  function appendChunk() {
    ...
    if (shown >= filtered.length) {
      ...
      endHint.textContent = `— 已加载全部 ${filtered.length.toLocaleString()} 张 —`;
    } else {
  ...
  function render() {
    ...
    countEl.innerHTML = active.length
      ? `共 <b>${filtered.length.toLocaleString()}</b> 张 · ${active.map(escapeHtml).join(" · ")}`
      : `共 <b>${filtered.length.toLocaleString()}</b> 张 / 全部 ${figures.length.toLocaleString()} 张`;
  }
